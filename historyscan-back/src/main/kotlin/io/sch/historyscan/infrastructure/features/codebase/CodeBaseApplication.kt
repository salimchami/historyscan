...
class CodeBaseApplication(private val clone: Clone,
    ...
    fun clone(codeBaseToAddDTO: CodeBaseToAddDTO): AddedCodebaseDTO {
        val codebase = clone.from(CodeBaseToClone(
    ...
    fun findCodeBase(codebaseName: String): CodebaseDTO {
        val codebase = findCodeBase.from(codebaseName)
        return codeBaseMapper.codebaseDomainToWeb(codebase)
    }
