...
class CodebaseRevisionsNetwork(private val history: CodeBaseHistory, clocRevisions: CodebaseClocRevisions) {
    ...
    val ignoredRevisions: List<io.sch.historyscan.domain.contexts.analysis.clocrevisions.filesystem.FileInfo> =
    ...
    fun calculateNetworkFromHistoryAndRevisions(codeBase: CodeBaseToAnalyze): CodebaseRevisionsNetwork {
        ...
        network = NetworkNodes(actualCodebaseFiles
            .map {
                NetworkNode(
                    it.name, it.path,
                    it.parentPath!!, it.currentNbLines,
                    it.revisionScore?.score!!, linksFrom(historyCommits, it)
                )
            }
            .sorted())
        return this
     ...
     */
    private fun filterHistoryFromActualFs(
        actualCodebaseFiles: List<FileSystemNode>,
        codebaseName: String
    ): List<CodeBaseCommit> {
                ...
                CodeBaseCommit(commit.info, commit.files.stream()
                    .filter { commitFile: CodeBaseHistoryCommitFile ->
                        actualCodebaseFiles.stream()
                            .anyMatch { actualFile: FileSystemNode ->
                                actualFile.path.contains(
                                    "%s/%s".format(
                                        codebaseName,
                                        commitFile.path()
                                    )
                    ...
                    .map { commitFile: CodeBaseHistoryCommitFile ->
                        CodeBaseHistoryCommitFile(
                            ...
                            commitFile.currentNbLines,
                            commitFile.nbAddedLines,
                            commitFile.nbDeletedLines,
                            commitFile.nbModifiedLines
                        )
                    }
                    .toList())
            }
